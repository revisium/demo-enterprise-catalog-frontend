...
| `/app`                  | Manage organization-specific runtime work   | Quotes, saved plans, favorites, audit trail             | Public catalog discovery, public docs browsing         | backend runtime                    |
...
- Account runtime proof should be split: workspace overview in `/app`, saved
  plan detail in `/app/plans/:planId`, and one authenticated quote conversation
  in `/app/quotes/:quoteId`.
