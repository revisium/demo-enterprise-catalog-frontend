          ...
          <FilterCard>
            ...
            {vm.relatedPlans.map((plan) => (
              <LinkedSummaryCard
                ...
                subtitle={`${plan.plan} · ${plan.region}`}
                subtitleFontSize="xs"
                title={plan.name}
        ...
        <Grid gap="4" templateColumns={{ base: '1fr', md: '120px minmax(0, 1fr)' }}>
          ...
          <Stack gap="3">
            ...
            {favoriteFetcher.data ? (
              <Text
                color={favoriteFetcher.data.status === 'accepted' ? 'successText' : 'amberText'}
                fontSize="sm"
              >
      ...
      <FilterCard>
        ...
        {vm.relatedQuotes.map((quote) => (
          <LinkedSummaryCard
            key={quote.id}
            returnState={returnState}
            subtitle={`${quote.status} · ${quote.commentCount} comments · $${quote.monthlyUsd}/mo`}
            subtitleFontSize="sm"
            title={`${quote.plan} · ${quote.region}`}
            titleFontSize="md"
